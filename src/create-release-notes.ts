...
const createReleaseNotes = (): void => {
  try {
    ...
    const outputPath = path.resolve(releasesDir, `${version}.md`);
    ...
    try {
      ...
      // Ignore errors if fetch fails (e.g. no remote)
    }
    ...
    const getAllCommits = (): string[] => {
      try {
        ...
        const allCommits = execSync(gitCommand, { stdio: ["pipe", "pipe", "ignore"] }).toString().trim();
    ...
    // Add initial message if content is mostly empty (caused by empty git history)
    const allCommits = getAllCommits();
